...
//	}
	...
	public boolean transfer(int fromAccount,double amount,String password,int toAccount) {
		...
		if(amount<=0 && !authenticate(password) )
			return false;
		if (amount>balance) {
			return false;
		} 
		...
		if(fromAccount==getAccountNumber()&&toAccount==getAccountNumber()) {
			//withdraw should take place from the fromAccount from accounts[]
			withdraw(amount,password);
			//deposit should take place to toAccount from accounts[]
			deposit(amount);
		}
		return false;
	}
	...
	public boolean openAccount(String string, String password2, double amount) {
		// TODO Auto-generated method stub
		return false;
	}
